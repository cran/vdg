...
#' @export plot spvlistforlist
plot.spvlistforlist <- function (x, which = 1L:5L, np = 50, alpha = 7/sqrt(length(x[[1]][[1]]$spv)), points.colour = "#39BEB1",
  ...
  if(show[1L]) {
    plot1 <- ggplot(tmp1, aes(x = Radius, y = SPV, order = Formula)) + 
  ...
  if(show[2L]){
    plot2 <- ggplot(tmp2, aes(x = Fraction, y = SPV, colour = Design)) + ggtitle("Fraction of Design Space Plot") + 
      ...
      theme(plot.title = element_text(size = rel(2), vjust = 1)) + facet_wrap(~ Formula)
    print(plot2)
  }
