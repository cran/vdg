...
#' @param which a vector of integers indicating which plots to produce. A
#' subset of \code{1L:4L} for \code{spv} or a subset of \code{1L:5L} for
#' \code{spvlist}
...
#' @return Returns a list of \code{\link{ggplot2}} graphical objects (or grobs) with names \code{plot1}, \code{plot2} etc. 
...
plot.spv <- function (x, which = 1L:5L, np = 50, alpha = 7/sqrt(length(x$spv)), points.colour = "#39BEB1",
                      ...
                      df = 10, lines.size = 1, 
                      origin = rep(0, ncol(x$sample)), method, ...) {
  ...
  if(any(show[-2L])) tmp1 <- data.frame(Radius = xvec, SPV = x$spv)
