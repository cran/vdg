...
#' @method plot spvlist
#' @export plot spvlist
plot.spvlist <- function (x, which = 1L:5L, np = 50, alpha = 7/sqrt(length(x[[1]]$spv)), points.colour = "#39BEB1",
                          points.size = 2, tau = c(0.05, 0.95), radii = 21, hexbin = FALSE, bins = 80, VRFDS = FALSE, 
  ...
  if(x[[1]]$at && show[2L]){
    show[2L] <- FALSE
    which <- which[!(which %in% 2L)]
    cat("Plot 2 cannot be produced: 'at' is TRUE (inaccurate FDS plot)\n")
  }
  ...
  if(!x[[1]]$at && show[5L]){
    ...
    cat("Plot 5 cannot be produced: 'at' is FALSE\n")
  }
  ...
  if(show[1L]) {
    plot1 <- ggplot(tmp1, aes(x = Radius, y = SPV, order = Design)) + 
  ...
  if(show[2L]) {
    plot2 <- ggplot(tmp2, aes(x = Fraction, y = SPV, colour = Design)) + ggtitle("Fraction of Design Space Plot") + 
      xlab("Fraction of Design Space") + geom_line(size = lines.size) +
      theme(plot.title = element_text(size = rel(2), vjust = 1))
    if(VRFDS) plot2 <- plot2 + ggtitle("Variance Ratio FDS Plot") + ylab(expression(log(SPV[x] / SPV[ref])))
  }
  ...
  if(show[4L]) {
    if(!exists("plot1", inherits = FALSE)){
      plot1 <- ggplot(tmp1, aes(x = Radius, y = SPV, order = Design)) + 
        ggtitle("Variance Dispersion Graph") +
        geom_point(alpha = alpha, colour = points.colour, size = points.size) + 
        theme(plot.title = element_text(size = rel(2), vjust = 1)) + 
    ...
    plot4 <- plot1 + geom_line(data = tmp3, aes(x = Radius, y = SPV, linetype = Location, 
...
if(length(which)) {
    ...
    if(VRFDS & show[2L]) out$plot2 <- out$plot2 + ylab(expression(log(SPV[x] / SPV[ref])))
    return(out)
  }
}