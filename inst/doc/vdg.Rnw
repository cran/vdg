...
\author{Pieter C. Schoonees\\Rotterdam School of Management, \\Erasmus University \And
        ...
        \And Roelof L. J. Coetzer\\Sasol Technology }
...
\Abstract{
This is an adjusted version of a paper submitted to the \emph{Journal of Statistical Software}.\\
...
Note that in~(\ref{E:InfoMat}) --~(\ref{E:spv}) only regression models that are
...
to characterize the spherical region moments~(\ref{E:sphMom}) in terms of 
...
This formula can be derived from the evaluation of~(\ref{E:sphMom}) after using
...
result in~(\ref{E:spvmom}), it is apparent that the spherical moments are 
...
Note that although~(\ref{E:spvmom}) facilitates the analytical calculation of
...
cuboidal, (\ref{E:spvmom}) does not hold for larger radii $r$ for which only
...
unscaled prediction variance (or UPV) instead of the SPV of Equation 
(\ref{E:spv}). The UPV $d^{*} (\mathbf{x}, \xi_{n} ) $ is defined as
\begin{equation}
...
In~(\ref{E:logVR}), $\xi_{n_{2}}^{2}$ is the reference  design and is
...
The \code{alpha} parameter determines the transparency of the plotted points, which helps to build a picture of the density of the prediction variance. It is often worth fine tuning this parameter, although a default is provided. The vector \code{tau} of values between zero and one determines which quantile regressions are included in the VDGs. A nonnull value for the \code{radii} argument implies that the mean spherical prediction variance will be added to the VDGs according to (\ref{E:spvmom}). It is advisable to read the help page of \code{meanspv()} when using this facility -- there are limits to what types of \code{formula}s can be handled safely. 
